...
/**
  * @brief  This function does an erase flash erea
  * @param  start: start address of user flash area need to be a page
...
uint32_t FLASH_If_Erase_Pages(uint32_t start_addr, uint32_t size)
{
  ...
  /* Get the number of page to  erase */
  NbrOfPages = size/FLASH_PAGE_SIZE;  //Calculate total page need to erase
  NbrOfPages += (start_addr - FLASH_START_ADRESS)/FLASH_PAGE_SIZE;

  if(NbrOfPages > FLASH_PAGE_NBPERBANK)//Bank 2
  {
    pEraseInit.Banks = GetBank(start_addr + size);
    pEraseInit.NbPages = (NbrOfPages % FLASH_PAGE_NBPERBANK);
  ...
  if(status == HAL_OK)
  {
	if(FLASH_PAGE_NBPERBANK > ((start_addr - FLASH_START_ADRESS)/FLASH_PAGE_SIZE))
	{
		NbrOfPages = size/FLASH_PAGE_SIZE;
	    pEraseInit.Banks = GetBank(start_addr);
	    pEraseInit.NbPages = NbrOfPages;
	    pEraseInit.Page = (start_addr - FLASH_START_ADRESS)/FLASH_PAGE_SIZE;
	    pEraseInit.TypeErase = FLASH_TYPEERASE_PAGES;
	    status = HAL_FLASHEx_Erase(&pEraseInit, &PageError);
	}
  }
...
uint32_t FLASH_If_Write_Bytes(uint32_t destination, uint32_t *p_source, uint32_t length)
{
  ...
  /* DataLength must be a multiple of 64 bit */
  for (i = 0; (i < length/8) && (destination <= (USER_FLASH_END_ADDRESS-8)); i++)
  {
    /* Device voltage range supposed to be [2.7V to 3.6V], the operation will
       be done by word */
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, destination, *((uint64_t *)(p_source+2*i))) == HAL_OK)
