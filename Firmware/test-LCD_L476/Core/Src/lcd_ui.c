...
void lcd_main_screen_screen(speaker_mode_t speaker_mode, int16_t temperature, power_mode_t power_mode, operation_mode_t operation_mode, uint8_t bat_value, battery_state_t bat_state)
{
	if(speaker_mode ==  SPEAKER_MODE_ON)
	{
		lv_obj_t* speaker_symbol_obj = get_label();
		lcd_label_set_param(speaker_symbol_obj, LV_SYMBOL_MUTE, -126, -89, &lv_font_montserrat_24, WHITE_COLOR);

	}
	lv_obj_t* temperature_obj = get_label();
	snprintf(temp_buff,MAX_TEMP_CHAR, "%d°C",temperature);
	lcd_label_set_param(temperature_obj, temp_buff, 103, -89, &lv_font_montserrat_24, WHITE_COLOR);

	lv_obj_t* operation_mode_obj = get_label();
	if(operation_mode == OPERATION_MODE_FREEZER)
	{
		lcd_label_set_param(operation_mode_obj, "Freezer", -2, -18, &ui_font_verdana404, WHITE_COLOR);
	}else
	{
		lcd_label_set_param(operation_mode_obj, "Fridge", -2, -18, &ui_font_verdana404, WHITE_COLOR);
	}
	lv_obj_t* power_mode_obj = get_label();
	if(power_mode)
	{
		lcd_label_set_param(power_mode_obj, "AC", -104, 63, &lv_font_montserrat_40, WHITE_COLOR);
	}else
	{
		lcd_label_set_param(power_mode_obj, LV_SYMBOL_BATTERY_FULL, -104, 63, &lv_font_montserrat_40, WHITE_COLOR);
	}

	lv_obj_t* bat_value_obj = get_label();

	snprintf(temp_buff, MAX_TEMP_CHAR, "%d",bat_value);
	lcd_label_set_param(bat_value_obj, temp_buff, 65, 62, &lv_font_montserrat_36, WHITE_COLOR);

	lv_obj_t* bat_state_obj = get_label();
	if(bat_state == BATTERY_STATE_CHARGING)
	{
		lcd_label_set_param(bat_state_obj, LV_SYMBOL_CHARGE, 108, 62, &lv_font_montserrat_36, WHITE_COLOR);
	}else
	{
		lcd_label_set_param(bat_state_obj, "%", 108, 62, &lv_font_montserrat_36, WHITE_COLOR);
	}
}
...
void lcd_service_data_logging_set(uint8_t value)
{
	...
	lcd_t time_1 = FONT_MONTSER_24;
	lcd_t time_2 = FONT_MONTSER_40;
	lcd_t time_3 = FONT_MONTSER_24;
...
void lcd_service_data_calibration_set(uint8_t value)
{
	...
	lcd_t temper_1 = FONT_MONTSER_24;
	lcd_t temper_2 = FONT_MONTSER_40;
	lcd_t temper_3 = FONT_MONTSER_24;
...
void lcd_service_temperature_fridge(service_temperature_fridge_t index, int8_t value)
{
	...
	lcd_t temper = FONT_MONTSER_24;
	lcd_t back = FONT_VERDENA_24;

	if(index == SERVICE_TEMPERATURE_FRIDGE_VALUE)
	{
		temper.font = &lv_font_montserrat_36;
	}else
	{
...
void lcd_service_alarm_bat(service_alarms_bat_t index, uint8_t value)
{
	...
	lcd_t temp_value = FONT_MONTSER_24;
	lcd_t back = FONT_VERDENA_24;

	if(index == SERVICE_ALARM_BAT_VALUE)
	{
		temp_value.font = &lv_font_montserrat_40;
	}else
	{
		back.font = &ui_font_verdana404;
	}
...
void lcd_service_alarm_bat_set(uint8_t value)
{
	...
	lcd_t percent_1 = FONT_MONTSER_24;
	lcd_t percent_2 = FONT_MONTSER_40;
	lcd_t percent_3 = FONT_MONTSER_24;
...
void lcd_service_alarm_lid(service_alarms_lid_t index, uint8_t value)
{
	...
	lcd_t time = FONT_MONTSER_24;
	lcd_t back = FONT_VERDENA_24;

	if(index == SERVICE_ALARM_LID_VALUE)
	{
		time.font = &lv_font_montserrat_40;
	}else
	{
		back.font = &ui_font_verdana404;
	}
...
void lcd_service_alarms_warning(warning_mode_t mode, warning_type_t type)
{
	...
	lcd_t warning_symbol = FONT_MONTSER_40;
	lcd_set_background_color(RED_COLOR);
...
//void lcd_turn_off_unit(bool value)
//{
//	lcd_t turn_off = FONT_VERDENA_36;
//	lcd_t yes = FONT_VERDENA_24;
//	lcd_t no = FONT_VERDENA_24;
//
//	if(value)
//	{
//		yes.font = &ui_font_verdana404;
//	}else
//	{
//		no.font = &ui_font_verdana404;
//	}
//
//
//
